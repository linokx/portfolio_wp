...
class EXP_Custom_Image_Footer {
	...
	function init() {
		...
		$this->page = $page = add_theme_page('Footer', 'Footer', 'edit_theme_options', 'esp-custom-footer', array(&$this, 'admin_page'));
		...
		$this->flinks = $this->theme_options['footer_links'];
	}
	 ...
	 */
	function help() {
		$help = '<p>You can set a custom image footer for your site. Simply upload the image and crop it, and the new footer will go live immediately.</p>' .
		'<p>If you want to discard your custom footer and go back to the default included in your theme, click on the buttons to remove the custom image and restore the original footer image.</p>';
		
		$screen = get_current_screen();

		if ( method_exists( $screen, 'add_help_tab' ) ) {
			$screen->add_help_tab( array(
				'title' => __( 'Overview', 'travelblogger' ),
	...
	function take_action() {
		...
		$this->theme_options['footer_links'] = $flinks;
...
<td>
	...
	<?php printf( 'Images of exactly <strong>%1$d &times; %2$d pixels</strong> will be used as-is.', FOOTER_IMAGE_WIDTH, FOOTER_IMAGE_HEIGHT ); ?></p>
