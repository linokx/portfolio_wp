...
function exp_load_theme_scripts() {
	wp_enqueue_script( 'exp-dependclass', get_stylesheet_directory_uri() . '/lib/admin/js/jquery.dependClass.js', false, '1.8.17', false );
	wp_enqueue_script( 'exp-jsui', get_stylesheet_directory_uri() . '/lib/admin/js/jquery.slider.js', false, '1.8.17', false );
...
function exp_update_style_sheet($options=null) {
 	$styles = $options['color_settings'];
	$fonts = $options['theme_font'];
	$data='';
...
function esp_admin_header_image() { ?>
	<div id="headimg">
		<?php
...
// Subclass Custom Header and Custom Background
function esp_custom_header_background() {
	...
	if ( ! function_exists( 'get_custom_header' ) ) {
		$admin_dev = 'esp_admin_header_image';
	}

	remove_action( 'admin_menu', array( &$custom_image_header, 'init' ) );
	remove_action( 'admin_menu', array( &$custom_background, 'init' ) );
	$custom_image_header = new EXP_Custom_Image_Header( 'esp_admin_header_style', $admin_dev );
...
add_action( 'wp_loaded', 'esp_custom_header_background' );
