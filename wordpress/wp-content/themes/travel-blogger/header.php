 ...
 */
	...
	if($exp_theme_options['featured_area'] == 'showall' || $exp_theme_options['featured_area'] == 'show' && (is_home() || is_category())) {
		$show_header = '1';
	} else {
		$show_header = '';
	}
	$grid = ($exp_theme_options['theme_grid'] == '') ? 'yui-t2' : $exp_theme_options['theme_grid'];
...
<body <?php body_class($exp_theme_options['theme_color']); ?>>
<div id="doc4" class="<?php exp_template_class($grid); ?>">
   <div id="hd">
  		<div id="header"> 
  			<div class="logo">
  				<?php if($exp_theme_options['show_header_text']) {?>
		...
		<?php if($exp_theme_options['social_bar']) { ?>
